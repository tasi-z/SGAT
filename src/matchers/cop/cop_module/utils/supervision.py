...
def spvs_coarse(data, config):
    ...
    data.update({'RESOLUTION':scale})
    scale0 = scale * data['scale0'][:, None] if 'scale0' in data else scale
    scale1 = scale * data['scale1'][:, None] if 'scale1' in data else scale
    h0, w0, h1, w1 = map(lambda x: x // scale, [H0, W0, H1, W1])
    data.update({'coarse_h0':h0,'coarse_w0':w0,'coarse_h1':h1,'coarse_w1':w1})
    ...
    #重叠区域掩码
    overlap_mask0 = torch.sum(conf_matrix_gt, dim=-1) >= 1
    #reshape
    overlap_mask0=overlap_mask0.reshape(N,1,h0,w0).float()
    # overlap_mask0=F.interpolate(overlap_mask0, size=(H0,W0), mode='bilinear', align_corners=False)
    overlap_mask1 = torch.sum(conf_matrix_gt, dim=-2) >= 1
    overlap_mask1=overlap_mask1.reshape(N,1,h1,w1).float()
    # overlap_mask1=F.interpolate(overlap_mask1, size=(H1,W1), mode='bilinear', align_corners=False)

    # img_np = overlap_mask1.long().squeeze().cpu().numpy()
    # plt.figure()
    # plt.imshow(img_np, cmap='gray')
    # plt.axis('off')
    # plt.savefig("temp.png")
    data.update({'overlap_mask0': overlap_mask0, 'overlap_mask1': overlap_mask1})
    ...
    data.update({
        'spv_b_ids': b_ids, #torch.Size([1295])
        'spv_i_ids': i_ids, #torch.Size([1295])
        'spv_j_ids': j_ids #torch.Size([1295])
    })

    # 6. save intermediate results (for fast fine-level computation)
    data.update({
        'spv_w_pt0_i': w_pt0_i, #torch.Size([1, 10816, 2])
        'spv_pt1_i': grid_pt1_i # torch.Size([1, 10816, 2])
    })
    coarse_corr0 = grid_pt0_i[b_ids, i_ids]  # Get keypoints from source image
    coarse_corr1 = grid_pt1_i[b_ids, j_ids]  # Get keypoints from target image
    data.update({'coarse_corr0': coarse_corr0, 'coarse_corr1': coarse_corr1})
@torch.no_grad()
def spvs_coarse_test(data, config):
    """
    Update:
        data (dict): {
            "conf_matrix_gt": [N, hw0, hw1],
    ...
    scale = 4
    data.update({'RESOLUTION':scale})
    scale0 = scale * data['scale0_test'][:, None] if 'scale0_test' in data else scale
    scale1 = scale * data['scale1_test'][:, None] if 'scale1_test' in data else scale
    ...
    # grid_pt0_i:b*grid_num*2
    #b_ids:match_num
    #i_ids:match_num
    #j_ids:match_num
    coarse_kpts0 = grid_pt0_i[b_ids, i_ids]  # Get keypoints from source image
    coarse_kpts1 = grid_pt1_i[b_ids, j_ids]  # Get keypoints from target image

    # Update data with the coarse keypoints
    data.update({
        'grid_pt0_i':grid_pt0_i,
        'grid_pt1_i':grid_pt1_i,
        'coarse_kpts0': coarse_kpts0,  # Keypoints in source image
        'coarse_kpts1': coarse_kpts1   # Keypoints in target image
    })
    data.update({'coarse_corr0': coarse_kpts0, 'coarse_corr1': coarse_kpts1})
