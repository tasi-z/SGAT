...
class LiFT(nn.Module):
    def __init__(self, in_channels, patch_size, out_channels=None, pre_shape=True, post_shape=True):
        ...
        self.outc = nn.Conv2d(in_channels//2, out_channels, kernel_size=1)
        if patch_size == 4:
            ...
            nn.Upsample(scale_factor=16/14, mode='bilinear', align_corners=False),
    ...
    def forward(self, imgs, x):
        ...
        if self.patch_size==14:
            b,_,h,w=imgs.shape
            target_h=h+self.patch_size-h%self.patch_size
            target_w=w+self.patch_size-w%self.patch_size
