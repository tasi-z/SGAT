...
def get_cosine_similarity(input1, input2):
    #注意默认的输入是（8,1000,256），即在调用函数时通常需要transport一下
    batch_size = input1.shape[0]
    feature_num = input1.shape[1]
    #L2 normalize
    norm_A = torch.norm(input1, dim=2, keepdim=True)
    norm_B = torch.norm(input2, dim=2, keepdim=True)
    input1 = input1 / norm_A
    input2 = input2 / norm_B
    #计算input1[0][0]的模
    # mo_input1=torch.sqrt(torch.sum(input1[0][0]**2))
    ...
    ortho_loss = []
    new_feature_dim=input1.shape[1]
    for i in range(input1.shape[0]):
        #原版考虑余弦相似性有正负号，进行平方
        c_sim=((input1_l2[i:i+1,:].mm(input2_l2[i:i+1,:].t())).pow(2))/(feature_num**2)
        #余弦相似性
        # c_sim=(input1_l2[i:i+1,:].mm(input2_l2[i:i+1,:].t()))/feature_num
        this_morth = torch.mean(c_sim)
        # ortho_loss += this_morth
        ortho_loss.append(this_morth)
    ortho_loss=torch.stack(ortho_loss)
...
class LightGlue(nn.Module):
    ...
    def __init__(self, conf) -> None:
        ...
        self.A_transformers = nn.ModuleList(
            [TransformerLayer(d, h, conf.flash) for _ in range(n)]
        )
        self.B_transformers = nn.ModuleList(
            [TransformerLayer(d, h, conf.flash) for _ in range(n)]
        )
        ...
        self.A_log_assignment = nn.ModuleList([MatchAssignment(d) for _ in range(n-1)])
        self.B_log_assignment = nn.ModuleList([MatchAssignment(d) for _ in range(n-1)])
    ...
    def forward(self, data: dict) -> dict:
        ...
        desc0_A,desc1_A=desc0_B,desc1_B=desc0,desc1
        ...
        all_desc0_A, all_desc1_A = [], []
        all_desc0_B, all_desc1_B = [], []
        c_sim0_tower,c_sim1_tower=[],[]
        ...
        for i in range(self.conf.n_layers):
            if self.conf.checkpointed and self.training:
                desc0_A, desc1_A = checkpoint(
                    self.A_transformers[i], desc0_A, desc1_A, encoding0, encoding1
                )
                desc0_B, desc1_B = checkpoint(
                    self.B_transformers[i], desc0_B, desc1_B, encoding0, encoding1
                )
            else:
                desc0_A, desc1_A = self.A_transformers[i](desc0_A,desc1_A, encoding0, encoding1)
                desc0_B, desc1_B = self.B_transformers[i](desc0_B,desc1_B, encoding0, encoding1)
            if self.training or i == self.conf.n_layers - 1:
                all_desc0_A.append(desc0_A)
                all_desc1_A.append(desc1_A)
                all_desc0_B.append(desc0_B)
                all_desc1_B.append(desc1_B)
                
                c_sim0=get_cosine_similarity(desc0_A,desc0_B)
                c_sim0_tower.append(c_sim0)
                c_sim1=get_cosine_similarity(desc1_A,desc1_B)
                c_sim1_tower.append(c_sim1)
                continue  # no early stopping or adaptive width at last layer
        
        desc0=desc0+self.cat_filter(torch.cat([desc0_A,desc0_B],-1))
        desc1=desc1+self.cat_filter(torch.cat([desc1_A,desc1_B],-1))

        desc0, desc1 = desc0[..., :m, :], desc1[..., :n, :]
        scores, _ = self.log_assignment(desc0, desc1)
        ...
        pred = {
            ...
            "ref_descriptors0_A": torch.stack(all_desc0_A, 1),
            "ref_descriptors1_A": torch.stack(all_desc1_A, 1),
            "ref_descriptors0_B": torch.stack(all_desc0_B, 1),
            "ref_descriptors1_B": torch.stack(all_desc1_B, 1),
    ...
    def loss(self, pred, data):
        def loss_params_A(pred, i):
            la_A, _ = self.A_log_assignment[i](
                pred["ref_descriptors0_A"][:, i], pred["ref_descriptors1_A"][:, i]
            )
            return {
                "log_assignment": la_A,
            }
        def loss_params_B(pred, i):
            la_B, _ = self.B_log_assignment[i](
                pred["ref_descriptors0_B"][:, i], pred["ref_descriptors1_B"][:, i]
            )
            return {
                "log_assignment": la_B,
            }
        orth_weight=0.1
        nll, gt_weights, loss_metrics = self.loss_fn(pred, data)
        N = pred["ref_descriptors0_A"].shape[1]
        losses = {"total": nll, "last": nll.clone().detach(), **loss_metrics}
        losses["orthloss"]=orth_weight*(0.5*pred['c_sim0'][-1]+0.5*pred['c_sim1'][-1]) 
        if self.training:
            losses["confidence"] = 0.0

        # B = pred['log_assignment'].shape[0]
        losses["row_norm"] = pred["log_assignment"].exp()[:, :-1].sum(2).mean(1)
        losses["c_sim_final"]=pred['c_sim0'][-1]
        sum_weights = 1.0
        for i in range(N - 1):
            params_i = loss_params_A(pred, i)
            nll, _, _ = self.loss_fn(params_i, data, weights=gt_weights)
            losses["c_sim_f"+str(i)]=pred['c_sim0'][i]
            ...
            # losses["orthloss"]+=(0.5*pred['c_sim0'][i]+0.5*pred['c_sim1'][i])*orth_weight    
            
            losses["confidence"] += self.token_confidence[i].loss(
                pred["ref_descriptors0_A"][:, i],
                pred["ref_descriptors1_A"][:, i],
                params_i["log_assignment"],
                pred["log_assignment"],
            ) / (N - 1)

            del params_i
        losses["total"] = losses["total"]/sum_weights
        
        sum_weights = 1.0
        for i in range(N - 1):
            params_i = loss_params_B(pred, i)
            nll, _, _ = self.loss_fn(params_i, data, weights=gt_weights)
            losses["c_sim_f"+str(i)]=pred['c_sim0'][i]
            if self.conf.loss.gamma > 0.0:
                weight = self.conf.loss.gamma ** (N - i - 1)
            ...
            losses["confidence"] += self.token_confidence[i].loss(
                pred["ref_descriptors0_B"][:, i],
                pred["ref_descriptors1_B"][:, i],
                params_i["log_assignment"],
                pred["log_assignment"],
            ) / (N - 1)
        ...
        losses["total"] = losses["total"]+losses["total"]/sum_weights
        
        # confidences
        if self.training:
            losses["total"] = losses["total"] + losses["confidence"]
            losses["total"] +=losses["orthloss"]
