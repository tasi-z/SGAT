...
from gluefactory.utils.upscale import upscale_keypoints
from gluefactory.models.matchers.msga_msn0 import MSGA
...
class TransformerLayer(nn.Module):
    ...
    def forward(
        ...
        if mask0 is not None and mask1 is not None:
            ...
            return desc0, desc1,self_sim0,self_sim1,cross_sim0,cross_sim1
...
class LightGlue(nn.Module):
    ...
    def __init__(self, conf) -> None:
        ...
        if conf.weights is not None:
            ...
            for i in range(self.conf.n_layers):
                ...
                state_dict = {k.replace(*pattern): v for k, v in state_dict.items()}
            if self.conf.n_layers<9:
                #删除所有层数大于n_layers的参数
                for k in list(state_dict.keys()):
                    if "log_assignment" in k or "transformers" in k:
                        if int(k.split(".")[1])>=self.conf.n_layers:
                            state_dict.pop(k)
                    if "token_confidence" in k:
                        if int(k.split(".")[1])>=self.conf.n_layers-1:
                            state_dict.pop(k)
    ...
    def forward(self, data: dict) -> dict:
        ...
        all_desc0, all_desc1 = [desc0], [desc1]
        ...
        for i in range(self.conf.n_layers):
            if self.conf.checkpointed and self.training:
                desc0, desc1, self_sim0, self_sim1, cross_sim0,cross_sim1 = checkpoint(
                    self.transformers[i], desc0, desc1, encoding0, encoding1
                )
            else:
                desc0, desc1, self_sim0, self_sim1, cross_sim0,cross_sim1 = self.transformers[i](desc0, desc1, encoding0, encoding1)
            # self_sim0_list.append(self_sim0)
            # self_sim1_list.append(self_sim1)
            # cross_sim0_list.append(cross_sim0)
            # cross_sim1_list.append(cross_sim1)
...
class MoEGlue(BaseModel):
    ...
    def _init(self, conf):
        ...
        dconf["n_layers"]=3
        self.lightglue_gim_s = LightGlue(dconf)
        self.lightglue_gim_s.requires_grad_(False)
        self.lightglue_gim_s.eval()
        dconf["weights"]="outputs/training/sp+msga_msn0_megadepth_231221/checkpoint_106_305000.tar"
        dconf["n_layers"]=9
        dconf["seed_top_k"]=[100,100]
        self.msn0=MSGA(dconf)
        self.set_initialized()
        self.loss_fn = NLLLoss(conf.loss)

    @torch.no_grad()
    def nn_upscale(self,res):
        kpts0, kpts1, matches = res["keypoints0"][0], res["keypoints1"][0], res["matches"][0]
        m_kpts0, m_kpts1 = kpts0[matches[..., 0]], kpts1[matches[..., 1]]
        upscale_m_kpts0, upscale_m_kpts1 = upscale_keypoints(m_kpts0, m_kpts1)
        res["keypoints0"]=upscale_m_kpts0.unsqueeze(0)
        res["keypoints1"]=upscale_m_kpts1.unsqueeze(0)
        upscale_indices = torch.arange(upscale_m_kpts0.shape[0])
        upscale_matches = upscale_indices.unsqueeze(1).repeat(1, 2)
        
        # Flatten the upscale_matches to conform to the shape needed
        res["matches"] = [upscale_matches]
        res["matches0"]=torch.arange(upscale_m_kpts0.shape[0]).unsqueeze(0)
        res["matches1"]=torch.arange(upscale_m_kpts0.shape[0]).unsqueeze(0)
        res["matching_scores0"]=torch.ones_like(res["matches0"])
        res["matching_scores1"]=torch.ones_like(res["matches1"])
        res["keypoint_scores0"]=torch.ones_like(res["matches0"])
        res["keypoint_scores1"]=torch.ones_like(res["matches1"])
        return res
    def _forward(self, data):
        with torch.no_grad():
            res_gim=self.lightglue_gim_s(data)
        new_data=data.copy()
        new_data["descriptors0"]=res_gim["ref_descriptors0"][:,-1,:,:]
        new_data["descriptors1"]=res_gim["ref_descriptors1"][:,-1,:,:]
        res=self.msn0(new_data)
        return res

    def loss(self, pred, data):
        def loss_params(pred, i):
            la, _ = self.msn0.log_assignment[i](
                pred["ref_descriptors0"][:, i], pred["ref_descriptors1"][:, i]
            )
            return {
                "log_assignment": la,
        ...
        for i in range(N - 1):
            ...
            losses["confidence"] += self.msn0.token_confidence[i].loss(
                pred["ref_descriptors0"][:, i],
                pred["ref_descriptors1"][:, i],
                params_i["log_assignment"],
                pred["log_assignment"],
