...
class LightGlue(nn.Module):
    ...
    def forward(self, data: dict) -> dict:
        ...
        if not self.training:
            ...
            gt_matching_scores0=result["matching_scores0"]
            gt_matching_scores1=result["matching_scores1"]
            #将kpts0中gt_matching_scores0为0的点坐标改为0,0,不可行，因为后面并不会因为坐标为0而不计算
            kpts0[gt_matching_scores0==0]=torch.tensor([0,0]).float().to(kpts0.device)
            kpts1[gt_matching_scores1==0]=torch.tensor([0,0]).float().to(kpts1.device)
