...
from .cop import full_default_cfg, opt_default_cfg, reparameter
...
from .dino_matchable import DINO_matchable
    ...
    def forward(
        ...
        if self.flash is not None and qk0.device.type == "cuda":
            ...
            if cross_mask0 is not None:
                weighted_sim_01=sim
                weighted_sim_10 = sim.transpose(-2, -1)
...
class TransformerLayer(nn.Module):
    ...
    def forward(
        ...
        if mask0 is not None and mask1 is not None:
            ...
            kpt_dino_feat0=data.get("kpt_dino_feat0",None)
            kpt_dino_feat1=data.get("kpt_dino_feat1",None)
            desc0=self.condition_attn(desc0,kpt_dino_feat0,kpt_dino_feat0)
            desc1=self.condition_attn(desc1,kpt_dino_feat1,kpt_dino_feat1)
...
class SPAGlue(BaseModel):
    default_conf = {"features": "superpoint", **SGAT.default_conf}
    required_data_keys = [
        "view0",
        "keypoints0",
        "descriptors0",
    ...
    def _forward(self, data):
        if not self.training:
            self.dino_backbone=reparameter(self.dino_backbone)
        ...
        with torch.no_grad():
            spec_mask0,spec_mask1,match_logits0_map,match_logits1_map,dino_data= self.dino_backbone(data["view0"],data["view1"])
            dino_feat0=dino_data["feat_c0_coarse"]
            dino_feat1=dino_data["feat_c1_coarse"]
            match_logits0_map=process_tensor(match_logits0_map,alpha=0.01,norm=True)
            match_logits1_map=process_tensor(match_logits1_map,alpha=0.01,norm=True)
            ...
            kpt_dino_feat0,kpt_dino_feat1= get_kpts_feat(dino_feat0,data["keypoints0"],size0),get_kpts_feat(dino_feat1,data["keypoints1"],size1)
        ...
        data["kpt_dino_feat0"]=kpt_dino_feat0
        data["kpt_dino_feat1"]=kpt_dino_feat1
        res=self.sgat(data)
