...
from ...settings import DATA_PATH, WEIGHTS_PATH
from ..utils.losses import NLLLoss
from ..utils.metrics import matcher_metrics
from ..utils.util import draw_torch_image,get_kpts_mask,draw_kpt_intensity,process_tensor,get_kpts_feat
...
class CondtionAttention(nn.Module):
    ...
    def forward(self, qurey, key, value, mask: Optional[torch.Tensor] = None,weighted_mask : Optional[torch.Tensor] = None
                ) -> torch.Tensor:
        # qurey:torch.Size([12, 2048, 256]) key.shape:torch.Size([12, 2048, 256]) value.shape:torch.Size([12, 2048, 256])
        q=self.Wq(qurey).unflatten(-1, (self.heads, -1)).transpose(1, 2)
        k=self.Wk(key).unflatten(-1, (self.heads, -1)).transpose(1, 2)
        v=self.Wv(value).unflatten(-1, (self.heads, -1)).transpose(1, 2)
        context = self.inner_attn(q, k, v, mask=mask)
        message = self.out_proj(context.transpose(1, 2).flatten(start_dim=-2))
        return qurey + self.ffn(torch.cat([qurey, message], -1))
...
class SelfBlock(nn.Module):
    ...
    def forward(
        ...
        # TODO: 可视化加权前后的注意力变化情况
            ...
            context = self.inner_attn(q, k, v, mask=mask,weighted_mask=learnable_weighted_mask)
        else:
            context = self.inner_attn(q, k, v, mask=mask)
        message = self.out_proj(context.transpose(1, 2).flatten(start_dim=-2))
...
class CrossBlock(nn.Module):
    ...
    def forward(
        self, x0: torch.Tensor, x1: torch.Tensor, mask: Optional[torch.Tensor] = None,cross_mask0: Optional[torch.Tensor] = None,cross_mask1: Optional[torch.Tensor] = None
    ) -> List[torch.Tensor]:
...
class TransformerLayer(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.condition_attn = CondtionAttention(*args, **kwargs)
    ...
    def forward(
        ...
        cross_mask0: Optional[torch.Tensor] = None,
        cross_mask1: Optional[torch.Tensor] = None,
    ):
        if mask0 is not None and mask1 is not None:
            return self.masked_forward(desc0, desc1, encoding0, encoding1, mask0, mask1)
        else:
            if weighted_mask0 is not None and weighted_mask1 is not None:
                # desc0 = self.self_attn(desc0, encoding0,weighted_mask=weighted_mask0)
                # desc1 = self.self_attn(desc1, encoding1,weighted_mask=weighted_mask1)
                desc0 = self.self_attn(desc0, encoding0)
                desc1 = self.self_attn(desc1, encoding1)
            else:
                desc0 = self.self_attn(desc0, encoding0)
                desc1 = self.self_attn(desc1, encoding1)
            kpt_cop_feat0=data.get("kpt_cop_feat0",None)
            kpt_cop_feat1=data.get("kpt_cop_feat1",None)
            desc0=self.condition_attn(desc0,kpt_cop_feat0,kpt_cop_feat0)
            desc1=self.condition_attn(desc1,kpt_cop_feat1,kpt_cop_feat1)
            return self.cross_attn(desc0, desc1,cross_mask0=cross_mask0,cross_mask1=cross_mask1)
...
class SGAT(nn.Module):
    default_conf = {
        "name": "sgat",  # just for interfacing
    ...
    def forward(self, data: dict) -> dict:
        ...
        spec_layer_num=3
        spec_kpt_mask0=data.get("spec_kpt_mask0",None)
        spec_kpt_mask1=data.get("spec_kpt_mask1",None)
        matchable_kpt_mask0=data.get("matchable_kpt_mask0",None)
        matchable_kpt_mask1=data.get("matchable_kpt_mask1",None)
        
        for i in range(self.conf.n_layers):
            if True:
                if self.conf.checkpointed and self.training:
                    desc0, desc1 = checkpoint(
                        self.transformers[i], desc0, desc1, encoding0, encoding1,data,None,None,spec_kpt_mask0,spec_kpt_mask1,matchable_kpt_mask0,matchable_kpt_mask1
                    )
                else:
                    desc0, desc1 = self.transformers[i](desc0, desc1, encoding0, encoding1,data=data,weighted_mask0=spec_kpt_mask0,weighted_mask1=spec_kpt_mask1,cross_mask0=matchable_kpt_mask0,cross_mask1=matchable_kpt_mask1)
            else:
                if self.conf.checkpointed and self.training:
                    desc0, desc1 = checkpoint(
                        self.transformers[i], desc0, desc1, encoding0, encoding1,data
                    )
                else:
                    desc0, desc1 = self.transformers[i](desc0, desc1, encoding0, encoding1,data=data)
...
class SGATMatcher(BaseModel):
    default_conf = {
        "features": "superpoint",
        "cop_weights": str(WEIGHTS_PATH / "cop/last.ckpt"),
        "sgat_weights": None,
        **SGAT.default_conf,
    }
    required_data_keys = [
    ...
    @staticmethod
    def _load_checkpoint_state_dict(weights_path):
    ...
    @classmethod
    def _looks_like_full_matcher_checkpoint(cls, state_dict):
        state_dict = cls._strip_prefix(state_dict, "module.")
        return any(
            k.startswith(prefix)
            for k in state_dict
            for prefix in ("matcher.", "extractor.")
        )

    @classmethod
    def _looks_like_full_matcher_state_dict(cls, state_dict):
        state_dict = cls._strip_prefix(state_dict, "module.")
        matcher_prefixes = ("cop_backbone.", "sgat.")
        return any(
            k.startswith(prefix)
            for k in state_dict
            for prefix in matcher_prefixes
        )

    @classmethod
    def _extract_matcher_state_dict(cls, state_dict):
        state_dict = cls._strip_prefix(state_dict, "module.")
        if any(k.startswith("matcher.") for k in state_dict):
            state_dict = {
                k.replace("matcher.", "", 1): v
                for k, v in state_dict.items()
    ...
    def _init(self, conf):
        dconf = OmegaConf.to_container(conf, resolve=True)
        checkpoint_state = None
        load_full_matcher_state = False
        sgat_init_weights = conf.sgat_weights if conf.get("sgat_weights") else None
        if conf.weights is not None:
            checkpoint_state = self._load_checkpoint_state_dict(conf.weights)
            if self._looks_like_full_matcher_checkpoint(checkpoint_state):
                checkpoint_state = self._extract_matcher_state_dict(checkpoint_state)
                load_full_matcher_state = True
                sgat_init_weights = None
            elif self._looks_like_full_matcher_state_dict(checkpoint_state):
                checkpoint_state = self._strip_prefix(checkpoint_state, "module.")
                load_full_matcher_state = True
                sgat_init_weights = None
            else:
                sgat_init_weights = conf.weights

        cop_weights = conf.cop_weights if conf.get("cop_weights") else None

        cop_config = deepcopy(full_default_cfg)
        cop_config["weights"] = cop_weights
        self.cop_backbone = COP(config=cop_config)
        self.cop_backbone.requires_grad_(False)
        self.cop_backbone.eval()

        dconf["weights"] = sgat_init_weights
        self.sgat = SGAT(dconf)
        if load_full_matcher_state:
            self.load_state_dict(checkpoint_state, strict=False)
        self.loss_fn = NLLLoss(conf.loss)
        # 除了condition_attn ，其余全部冻结
        # for name, param in self.sgat.named_parameters():
        #     if "condition_attn" in name :
        #     # if "transformers" in name or "log_assignment" in name :
    ...
    def _forward(self, data):
        if not self.training:
            self.cop_backbone=reparameter(self.cop_backbone)
        ...
        with torch.no_grad():
            spec_mask0,spec_mask1,match_logits0_map,match_logits1_map,cop_data= self.cop_backbone(data["view0"],data["view1"])
            # spec_mask0=spec_mask0*match_logits0_map
            # spec_mask1=spec_mask1*match_logits1_map
            cop_feat0=cop_data["feat_c0_coarse"]
            cop_feat1=cop_data["feat_c1_coarse"]
            match_logits0_map=process_tensor(match_logits0_map,alpha=0.001,norm=True)
            match_logits1_map=process_tensor(match_logits1_map,alpha=0.001,norm=True)
            # spec_mask0=match_logits0_map*spec_mask0
            # spec_mask1=match_logits1_map*spec_mask1
            spec_mask0=process_tensor(match_logits0_map*spec_mask0,alpha=0,norm=True)
            spec_mask1=process_tensor(match_logits1_map*spec_mask1,alpha=0,norm=True)
            spec_kpt_mask0,spec_kpt_mask1= get_kpts_mask(spec_mask0,data["keypoints0"]),get_kpts_mask(spec_mask1,data["keypoints1"])
            matchable_kpt_mask0,matchable_kpt_mask1= get_kpts_mask(match_logits0_map,data["keypoints0"]),get_kpts_mask(match_logits1_map,data["keypoints1"])
            kpt_cop_feat0,kpt_cop_feat1= get_kpts_feat(cop_feat0,data["keypoints0"],size0),get_kpts_feat(cop_feat1,data["keypoints1"],size1)
        # spec_kpt_mask0=None
        # spec_kpt_mask1=None
        data["spec_kpt_mask0"]=spec_kpt_mask0
        data["spec_kpt_mask1"]=spec_kpt_mask1
        data["matchable_kpt_mask0"]=matchable_kpt_mask0
        data["matchable_kpt_mask1"]=matchable_kpt_mask1
        data["kpt_cop_feat0"]=kpt_cop_feat0
        data["kpt_cop_feat1"]=kpt_cop_feat1
        res=self.sgat(data)
    ...
    def loss(self, pred, data):
        def loss_params(pred, i):
            la, _ = self.sgat.log_assignment[i](
        ...
        for i in range(N - 1):
            ...
            losses["confidence"] += self.sgat.token_confidence[i].loss(
