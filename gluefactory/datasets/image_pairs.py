...
class ImagePairs(BaseDataset, torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # if self.dataset_name=="Scannet1500":
        #     print(self.dataset_name)
        data = {
            "view0": data0,#scales,image_size,transform,original_image_size,image
            "view1": data1,
        }
        if self.conf.extra_data == "relative_pose":
            # data["view0"]["camera"] = parse_camera(pair_data[2:11])
            # data["view1"]["camera"] = parse_camera(pair_data[11:20])
            if self.dataset_name=="Scannet1500" or self.dataset_name=="eth3d1500":
                data["view0"]["camera"] = parse_camera(pair_data[2:11])
                data["view1"]["camera"] = parse_camera(pair_data[11:20])
            else:
