...
from ..geometry.homography import homography_corner_error,get_pck,homography_pck_error, sym_homography_error,homography_corner_error_all
...
def eval_matches_epipolar(data: dict, pred: dict) -> dict:
    ...
    results["epi_prec_detail"]= np.array(n_epi_err)
    results["epi_prec@1e-4"] = (n_epi_err < 1e-4).float().numpy().mean()
    results["epi_prec@5e-4"] = (n_epi_err < 5e-4).float().numpy().mean()
    results["epi_prec@1e-3"] = (n_epi_err < 1e-3).float().numpy().mean()
...
def eval_relative_pose_robust(data, pred, conf):
    ...
    if not est["success"]:
        ...
        results["ransac_inl%"] = 0
        results["ransac_inl_detail"] = np.array([])
    else:
        # R, t, inl = ret
        M = est["M_0to1"]
        ...
        results["ransac_inl_detail"] = np.array(inl)

    return results


def eval_homography_robust(data, pred, conf,error_type="corner"):
    H_gt = data["H_0to1"]
    if H_gt.ndim > 2:
    ...
    if est["success"]:
        M = est["M_0to1"]
        if error_type=="corner":
            error_r = homography_corner_error(M, H_gt, data["view0"]["image_size"]).item()
        elif error_type=="all":
            error_r = homography_corner_error_all(M, H_gt, data["view0"]["image_size"]).item()
        elif error_type=="pck":
            error_r=homography_pck_error(M, H_gt, data["view0"]["image_size"])
    else:
        M=torch.zeros_like(H_gt)
        error_r = float("inf")
        error_affine={}
    results = {}
    results["est_H"] = M.reshape(-1).cpu().numpy()
    results["H_error_ransac"] = error_r
    # results["H_error_affine"] = error_affine.get("H_error",0)
    # results["alpah_1_rightnum"] = error_affine.get("alpha_1_rightnum",0)
    # results["alpah_3_rightnum"]=error_affine.get("alpha_3_rightnum",0)
    # results["alpah_5_rightnum"]=error_affine.get("alpha_5_rightnum",0)
    # results["affine_num"]=error_affine.get("affine_num",0)
    # results["max_l"]=error_affine.get("max_l",0)
    if "inliers" in est:
        inl = est["inliers"]
        results["ransac_inl"] = inl.float().sum().item()
        results["ransac_inl%"] = inl.float().sum().item() / max(len(inl), 1)
        # 保存 RANSAC 剔除后的匹配点
        inlier_mask = inl.bool().flatten()
        results["matched_pts0"] = pts0[inlier_mask].cpu().numpy()
        results["matched_pts1"] = pts1[inlier_mask].cpu().numpy()
...
def eval_poses(pose_results, auc_ths, key, unit="°",pck_th=None):
    pose_aucs = {}
    pck_res={}
    best_th = -1
    for th, results_i in pose_results.items():
        pose_aucs[th] = AUCMetric(auc_ths, results_i[key]).compute()
        if pck_th is not None:
            affine_num=np.array(results_i["affine_num"]).sum()
            for th in pck_th:
                max_l=results_i["max_l"][0]*th
                pck_res[th]=get_pck(max_l,results_i["H_error_affine"],affine_num)
            
    mAAs = {k: np.mean(v) for k, v in pose_aucs.items()} #AUC的平均值
    ...
    if pck_th is not None:
        for i, ath in enumerate(pck_th):
            summaries[f"pck@{ath}"] = pck_res[ath]
    summaries[f"{key}_mAA"] = mAAs[best_th]

    for k, v in pose_results[best_th].items():
        if k in ["H_error_affine", "max_l", "affine_num", "ransac_inl_detail", "matched_pts0", "matched_pts1"]:
            continue
