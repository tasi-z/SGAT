...
def cal_error_auc(errors, thresholds):
    sort_idx = np.argsort(errors)#将pose_error从小到大排序
    errors = np.array(errors.copy())[sort_idx]
    recall = (np.arange(len(errors)) + 1) / len(errors)#计算召回率（recall）
    errors = np.r_[0.0, errors]
    recall = np.r_[0.0, recall]
    aucs = []
    for t in thresholds:
        last_index = np.searchsorted(errors, t) #即返回第一个大于等于 t 的数的索引
        r = np.r_[recall[:last_index], recall[last_index - 1]]# 生成一个新的数组，其中包含 recall[:last_index] 和 recall[last_index - 1] 的元素
        e = np.r_[errors[:last_index], t]
        # NumPy 2.x removes np.trapz in favor of np.trapezoid.
        integrate = np.trapezoid if hasattr(np, "trapezoid") else np.trapz
        aucs.append(np.round((integrate(r, x=e) / t), 4))
    return aucs
