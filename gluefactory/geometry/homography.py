...
def homography_pck_error_reverse(T, T_gt, image_size):
    ...
    grid = torch.stack((grid_x, grid_y), dim=-1).float().to(T_gt)

    # 将网格转换为齐次坐标
    # 1图的坐标
    grid_homogeneous_1 = to_homogeneous(grid.view(-1, 2))
    H0to1=np.array(T)
    H1to0=np.linalg.inv(H0to1)
    # 单应逆向变化
    # 0图的坐标
    grid_homogeneous_0=to_homogeneous(from_homogeneous(grid_homogeneous_1@H1to0.transpose(-1, -2)))
    # 应用预测和真实的单应性变换
    transformed_grid = from_homogeneous(grid_homogeneous_0 @ T.transpose(-1, -2))
    transformed_grid_gt = from_homogeneous(grid_homogeneous_0 @ T_gt.transpose(-1, -2))

    # 计算误差
    error = torch.sqrt(((transformed_grid - transformed_grid_gt) ** 2).sum(-1))

    # 重新调整误差图的形状
    error_map = error.view(H, W)
    #将nan映射为255
    error_map=error_map.nan_to_num(255)
    return error_map
...
def homography_corner_error_all(T, T_gt, image_size):
    W, H = image_size[..., 0], image_size[..., 1]
    gt_overlay=homography_pck_error_reverse(T_gt,T,(W,H))
    return gt_overlay.mean()

def get_pck(alpha_l,error,affine_num):
    right_num=0
    for item in error:
        right_num+=(item<alpha_l).sum()
    pck=right_num/affine_num
    return pck

def homography_pck_error(T, T_gt, image_size):
    W, H = image_size[..., 0], image_size[..., 1]
    #20个点
    aff_points = [[0, 0], [W, 0], [W, H], [0, H], [W/2, H/2], [W/4, H/4], [W/4, H*3/4], [W*3/4, H/4], [W*3/4, H*3/4], [W/8, H/8], [W/8, H*7/8], [W*7/8, H/8], [W*7/8, H*7/8], [W/2, H/4], [W/2, H*3/4], [W/4, H/2], [W*3/4, H/2], [W/4, H/2], [W*3/4, H/2], [W/2, H/8]]
    corners0 = torch.Tensor(aff_points).float().to(T)
    corners1_gt = from_homogeneous(to_homogeneous(corners0) @ T_gt.transpose(-1, -2))
    corners1 = from_homogeneous(to_homogeneous(corners0) @ T.transpose(-1, -2))
    d = torch.sqrt(((corners1 - corners1_gt) ** 2).sum(-1))
    max_l=max(W,H).numpy()
    # alpha_1 = 0.01*max_l
    # alpha_1_rightnum=(d<alpha_1).sum()
    # alpha_3 = 0.03*max_l
    # alpha_3_rightnum=(d<alpha_3).sum()
    # alpha_5 = 0.05*max_l
    # alpha_5_rightnum=(d<alpha_5).sum()
    affine_num=20
    d = d.numpy()
    res={
        #  "alpha_1_rightnum":alpha_1_rightnum,
        # "alpha_3_rightnum":alpha_3_rightnum,
        # "alpha_5_rightnum":alpha_5_rightnum,
        "affine_num":affine_num,
        "H_error":d,
        "max_l":max_l
    }
    return res