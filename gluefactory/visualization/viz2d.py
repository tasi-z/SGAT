...
def plot_cumulative(
    ...
    fontsize=20,
    logx=False,
    return_acc_dict=False,
    figsize=(20,20),
    with_text=True,
    specific_xs=[1,3, 5, 10, 15,20,25,30],
    linewidth=1,
    ylim=[0,100],
    ...
    texts_dict = {}
    acc_dict = {}
    for i,method in enumerate(errors):
        recall = []
        ...
        # 添加特定横坐标的纵坐标值标识
        for x in specific_xs:
            if min(thresholds) <= x <= max(thresholds):
                y = np.interp(x, thresholds, recall)
                if method not in acc_dict:
                    acc_dict[method] = {}
                acc_dict[method][x] = f"{y:.1f}%"
                if with_text:
                    text =plt.annotate(
                        f"{y:.1f}%",
                        (x, y),
                        ...
                        ha='center',
                    fontsize=fontsize,
                    color=colors[method] if colors else 'black'
                    )
                    if x not in texts_dict:
                        texts_dict[x] = []
                    texts_dict[x].append(text)
        # 绘制垂直虚线
                    plt.axvline(x=x, color='gray', linestyle='--', linewidth=linewidth)
    ...
    for k,texts in texts_dict.items():
        if with_text:
            adjust_text(texts,arrowprops=dict(arrowstyle='->', color='red',lw=4))
        # break
